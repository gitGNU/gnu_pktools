...
namespace filter
{
  ...
  enum FILTER_TYPE { median=0, var=1 , min=2, max=3, sum=4, mean=5, minmax=6, dilate=7, erode=8, close=9, open=10, homog=11, sobelx=12, sobely=13, sobelxy=14, sobelyx=-14, smooth=15, density=16, mode=17, mixed=18, smoothnodata=19, threshold=20, ismin=21, ismax=22, heterog=23, order=24, stdev=25, dwt=26, dwti=27, dwt_cut=28, dwt_cut_from=29, savgolay=30, percentile=31, nvalid=32, first=33, last=34};
  ...
  public:
    ...
    void morphology(ImgRasterGdal& input, ImgRasterGdal& output, const std::string& method, int dim, short verbose=0);
    void filter(ImgRasterGdal& input, ImgRasterGdal& output);
    void stat(ImgRasterGdal& input, ImgRasterGdal& output, const std::string& method);
    void stats(ImgRasterGdal& input, ImgRasterGdal& output, const std::vector<std::string >& methods);
    void filter(ImgRasterGdal& input, ImgRasterGdal& output, const std::string& method, int dim);
    ...
    /* void savgolay(const std::shared_ptr<ImgRasterGdal> input, std::shared_ptr<ImgRasterGdal> output, int np, int nl, int nr, int m); */
    void smooth(ImgRasterGdal& input, ImgRasterGdal& output, short dim);
    void smoothNoData(ImgRasterGdal& input, const std::string& interpolationType, ImgRasterGdal& output);
    ...
    void dwtCut(ImgRasterGdal& input, ImgRasterGdal& output, const std::string& wavelet_type, int family, double cut);
    ...
    void dwtCutFrom(ImgRasterGdal& input, ImgRasterGdal& output, const std::string& wavelet_type, int family, int band);
  ...
  template<class T> void Filter::filter(T* input, int inputSize, std::vector<T>& output)
    {
        ...
        for(int t=1;t<=m_taps.size()/2;++t){
          ...
          if(i>=t)
            ...
            case(replicate):
              ...
              break;
            case(circular):
              output[i]+=m_taps[m_taps.size()/2-t]*input[inputSize+i-t];
              break;
            case(zero):
              output[i]+=m_taps[m_taps.size()/2-t]*0;
              break;
      ...
      //main
      for(i=m_taps.size()/2;i<inputSize-m_taps.size()/2;++i){
        //todo:introduce nodata
        T leaveOut=(*(m_taps.begin()))*input[i-m_taps.size()/2];
        T include=(m_taps.back())*input[i+m_taps.size()/2];
        output[i]=0;
