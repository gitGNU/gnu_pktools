...
CPLErr ImgRaster::stretch(ImgRaster& imgWriter, const AppFactory& app){
  ...
  try{
    ...
    if(verbose_opt[0]){
      std::cout << "cc_min: " << minp_opt[0] << std::endl;
      std::cout << "cc_max: " << maxp_opt[0] << std::endl;
    }
    if(band_opt.empty()){
    ...
    for(int iband=0;iband<band_opt.size();++iband){
      ...
      double value=0;
      if(verbose_opt[0])
        std::cout << "Histogram (cumulative): " << std::endl;
      int ibin=0;
      ...
      for(int irow=0;irow<nrOfRow();++irow){
        ...
        for(int icol=0;icol<nrOfCol();++icol){
          if(isNoData(lineBuffer[icol]))
            ...
            double normValue=histogramOutput.back()/100.0;
            double cdf_min=histogramOutput[0]/normValue;
            if(cdf_min<minp_opt[0])
              cdf_min=minp_opt[0];
            double cdf_max=maxp_opt[0];
            double cdf=histogramOutput[theBin]/normValue;
            if(cdf<cdf_min)
              cdf=cdf_min;
            if(cdf>cdf_max)
              cdf=cdf_max;
            double value=(cdf-cdf_min)/(cdf_max-cdf_min)*(toValue_opt[0]-fromValue_opt[0])+fromValue_opt[0];
            // double value=(histogramOutput[theBin]-histogramOutput[0])/(nvalid-histogramOutput[0])*(toValue_opt[0]-1-fromValue_opt[0])+fromValue_opt[0];
